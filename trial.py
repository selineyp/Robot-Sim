...
def extractExtensions(answerset):
  ...
  row_pattern = re.compile(r'(\brow(\b)*(\d+)*(\b)\b)',str1)
  col_pattern = re.compile(r'(\bcolumn(\b)*(\d+)*(\b)\b)',str1)
  #print(row_pattern)
  #print(col_pattern)
  str2=re.findall(row_pattern,str1)
  print(str2)
  '''
def extractExtensions(answerset):
  #print(repr(answer_set))
...
def render_svg(literals,size=20):
    ...
    #maxx = max([x[0] for x in answer_set['field']])
    #maxy = max([x[1] for x in answer_set['field']])
